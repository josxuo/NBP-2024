...
## most support for nbiom2. Compare agains null models
m01 <- glmmTMB(maps ~ 1, data = focal.dat, family = nbinom2)
m02 <- glmmTMB(maps ~ 1 + (1 | station.code), data = focal.dat, family = nbinom2)

AICtab(m3, m01, m02, mnames = c("m3", "m01", "m02")) ## M3 still supported
...
ggplot(mean.maps, aes(x = year, y = mean)) +
  ...
  annotate("text", x = 2002, y = max(mean.maps$mean)+.02, label = paste0("Mean abundance per survey decreasing by ", 
