...
# MOVING ON
# All species recorded in dataset, including rarities, hybrids, subspecies
S1 <- unique(raw[!str_detect(raw$Species, pattern = " sp\\."), ]$Species)
sort(S1)

## All species recorded by excluding spurious spotted owl entry
...
## All observation lumped in current taxonomic status, and hybrids but excluding species observed on just a single occasion
S4 <- dat[!str_detect(dat$species, pattern = " sp\\.|Spotted Owl"), ] %>%
  group_by(species) %>% reframe(n = n()) %>% filter(n > 1)

view(S4)

## All observations lumped in current taxonomic status and hybrids, excluding birds observed on fewer than five occasions
S5 <- dat[!str_detect(dat$species, pattern = " sp\\.|Spotted Owl"), ] %>%
  group_by(species) %>% reframe(n = n()) %>% filter(n > 4)
view(S5)

dim(S4)[1]-dim(S5)[1]

## All observation lumped in current taxonomic status, excluding rarities and excluding hybrids
S6 <- dat[!str_detect(dat$species, pattern = " sp\\.|Spotted Owl| x "), ] %>%
  group_by(species) %>% reframe(n = n()) %>% filter(n > 4)
view(S6)

## filter data for observations resolved to species level (dres) excluding spurious Spotted Owl entry
...
## This above is a summary of the entire dataset. For trend analysis, we need to compare sites that have been consistently monitored over the same time frame
...
## Most species on a single loop
dres %>% group_by(park, loop, survey_date) %>% reframe(S = n_distinct(species),
                                                       ncirc = n_distinct(station.code)) %>%
  arrange(desc(S))


## Greatest number of birds counted on a singel survey
## use all birds, even sp. and spurious
dat %>% group_by(survey_id) %>% reframe(nbirds = sum(seen, heard, fly)) %>%
  arrange(desc(nbirds))

Greatest number of birds on a single loop
dat %>% group_by(park, loop, survey_date) %>% reframe(nbirds = sum(seen, heard, fly), S = n_distinct(species)) %>%
  arrange(desc(nbirds))

view(dat[dat$park == "Genesee Park" & dat$year == 2023 & dat$month == 3, ])

## most frequenlty reported species
nsurv <- length(unique(dat$survey_id))

dat %>% group_by(species) %>% reframe(prop = n() / nsurv) %>% arrange(desc(prop))


## most abundant species
dat %>% group_by(species, survey_id) %>% reframe(nind = sum(seen, heard, fly)) %>%
  filter(nind == max(nind))
...
for(i in 1:length(sp.list)){
  plot(maps.mon[maps.mon$species == sp.list[i], ]$month, maps.mon[maps.mon$species == sp.list[i], ]$maps, 
       ...
       xlim = c(1, 12), yaxt = "n", xaxt = "n")
    axis(1, at = seq(1, 12, by = 1), las = 1)
}

plot(maps.mon[maps.mon$species == "American Goldfinch", ]$month, maps.mon[maps.mon$species == "American Goldfinch", ]$maps, type = "l")
...
avgS <- dSprk %>% filter(status == "active") %>% summarise(mean = mean(S)) %>% pull(mean)

pSprks <- ggplot(filter(dSprk, status == "active"), aes(x = park, y = S)) +
  geom_col(fill = bcs_colors["dark green"]) +
  coord_flip() + 
  labs(x = "", y = "Species richness", title = "Species richness",
       subtitle = "active NBP survey locations") + 
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  theme_bcs() + 
  theme(panel.grid.major.y = element_blank()) +
  theme(axis.title = element_text(size = 14), 
        axis.text = element_text(size = 12)) + 
  geom_hline(yintercept = avgS, color = bcs_colors["bright green"], lty = 2, linewidth = 1.5) +
  annotate("text", x = 0.8, y = avgS + 30, label = paste("mean =", round(avgS, 0)), color = bcs_colors["dark green"], size = 5)
...
# pdf("figures/03_total_S_by_Park.pdf", width = 9)
# print(pSprks)
# dev.off()
